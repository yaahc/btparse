...
$(rustc --print target-libdir)/../bin/llvm-cov show -format=html -Xdemangler=rustfilt -instr-profile=./target/test.profdata $(find target/debug/deps -type f -perm -u+x ! -name '*.so') -show-regions -show-instantiations
